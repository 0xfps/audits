...
contract TheRewarderPool {
    ...
    function distributeRewards() public returns (uint256 rewards) {
        ...
        /// @dev Total supply at last reward time.
        uint256 totalDeposits = accountingToken.totalSupplyAt(lastSnapshotIdForRewards);
        /// @dev
        /// @audit  Should return the balance at the last time which will be
        ///         subtracted with the current time to know amountDeposited.
        ///         Returns ???
    ...
    function _hasRetrievedReward(address account) private view returns (bool) {
        return (
            lastRewardTimestamps[account] >= lastRecordedSnapshotTimestamp /// @audit returns false because 0 !>= uint64...
                && lastRewardTimestamps[account] <= lastRecordedSnapshotTimestamp + REWARDS_ROUND_MIN_DURATION /// @audit Should've been >=.
        );
    }

    function isNewRewardsRound() public view returns (bool) {
        /// @dev If time is >= 5 days.
