...
contract Exchange is ReentrancyGuard {
    ...
    function buyOne() external payable nonReentrant returns (uint256 id) {
        ...
        uint256 price = oracle.getMedianPrice(token.symbol()); /// @audit A1 Any way to tamper with median price?
        if (msg.value < price)
            revert InvalidPayment();
    ...
    function sellOne(uint256 id) external nonReentrant {
        if (msg.sender != token.ownerOf(id)) /// @audit Token must be bought first before sale.
